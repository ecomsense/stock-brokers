...
class SmartWebSocketOrderUpdate(object):
    ...
    def __init__(self, auth_token, api_key, client_code, feed_token):
        ...
        logzero.logfile(
            log_path, loglevel=logging.INFO
    ...
    def on_ping(self, wsapp, data):
        ...
        logger.info(
            "In on ping function ==> %s, Timestamp: %s", data, formatted_timestamp
        )
    ...
    def on_pong(self, wsapp, data):
        if data == self.HEARTBEAT_MESSAGE:
            ...
            logger.info(
                "In on pong function ==> %s, Timestamp: %s", data, formatted_timestamp
            )
