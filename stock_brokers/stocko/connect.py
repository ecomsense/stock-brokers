...
class Connect:
    ...
    def place_order(self, payload):
        data = {
            ...
            "instrument_token": payload['instrument_token'],    #4717
            "client_id": payload['client_id'],                  #"XYZ"
            "order_type": payload['order_type'],                #"LIMIT" 
            "amo": payload['amo'],                              #False
            "price": payload['price'],                          #REQUIRED IF NOT MARKET ORDER
            "quantity": payload['quantity'],                     #1
            "disclosed_quantity": payload['disclosed_quantity'], #0
            "validity": payload['validity'],                     #"DAY"
            "product": payload['product'],                       #"MIS"
            "order_side": payload['order_side'],                  #BUY SELL
            "device": "api",
            "user_order_id": payload['user_order_id'],            #1002
            "trigger_price": payload['trigger_price'],                  #None
            "execution_type": payload['execution_type']             #"REGULAR"
        }
